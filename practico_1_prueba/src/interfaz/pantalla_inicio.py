...
class PantallaInicio(QWidget):
    ...
    def init_ui(self):
        ...
        self.boton_normalizar = QPushButton("Normalizar Colores")
        self.boton_normalizar.clicked.connect(self.normalizar_imagen)
        self.boton_normalizar.setIcon(QIcon(str(ICONS_DIR / "normalize_colors.svg")))
        self.boton_normalizar.setIconSize(QSize(24, 24))
        botones_layout.addWidget(self.boton_normalizar)

        self.boton_rgb_a_yiq = QPushButton("RGB → YIQ")
        self.boton_rgb_a_yiq.clicked.connect(self.convertir_rgb_a_yiq)
        botones_layout.addWidget(self.boton_rgb_a_yiq)
        self.boton_normalizar.setIcon(QIcon(str(ICONS_DIR / "YIQ_colors.svg")))
        self.boton_normalizar.setIconSize(QSize(24, 24))

        self.boton_procesar_yiq = QPushButton("Procesar YIQ")
        self.boton_procesar_yiq.clicked.connect(self.procesar_yiq_imagen)
        self.boton_normalizar.setIcon(QIcon(str(ICONS_DIR / "YIQ_Process.svg")))
        self.boton_normalizar.setIconSize(QSize(24, 24))
        botones_layout.addWidget(self.boton_procesar_yiq)
    ...
    def procesar_yiq_imagen(self):
        if self.imagen_actual:
        # Podés ajustar a y b como parámetros
            self.imagen_actual = GestorImagen.procesar_yiq(self.imagen_actual, a=1.2, b=0)
            self.mostrar_imagen_actual()

    
    
    def convertir_rgb_a_yiq(self):
        if self.imagen_actual:
            self.imagen_actual = GestorImagen.rgb_a_yiq(self.imagen_actual)
            self.mostrar_imagen_actual()



    def normalizar_imagen(self):
        if self.imagen_actual:
            self.imagen_actual = GestorImagen.normalizar_imagen(self.imagen_actual)
            self.mostrar_imagen_actual
